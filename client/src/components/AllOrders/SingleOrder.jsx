...
import {
  ...
  sortByPrice,
  sortByCost,
} from "../OrderHistory/FilterOrderDetail";
...
function SingleOrder({ order, modify }) {
  ...
  function handleSubmit(e) {
    ...
    swal({
      ...
      text:
        "Una vez modificada, debera refrescar la pagina para ver los cambios!",
      icon: "warning",
      buttons: true,
      dangerMode: true,
      ...
      if (willDelete) {
        await axios.post(
          `http://localhost:3001/orders/${order.id}?state=${status}`
        );

        swal(
          "Éxito!",
          `La orden ${order.id} ha sido modificada.`,
          "success"
        ).then(async (e) => {
          if (status === "completed") {
            await axios.post(
              `http://localhost:3001/email/order-notification`,
              {
  ...
  let statusForm = () => {
    if (modify && order.state === "created") {
      ...
      return <DivText content="Creada" />;
    } else if (status === "processing") {
      return <DivText content="Procesando" />;
    } else if (status === "cancelled") {
      return <DivText content="Cancelada" />;
    } else if (status === "completed") {
      return <DivText content="Completada" />;
    }
  };
        ...
        <div className={activeToggle}>
          <div className="orderAdmin">
            ...
            <div className="orderAdminCreatedAt" onClick={toggle}>
              <DivText
                content={order.createdAt.substring(0, 19).split("T").join(" ")}
              />
            </div>
            <div className="orderAdminUpdatedAt" onClick={toggle}>
              <DivText
                content={order.updatedAt.substring(0, 19).split("T").join(" ")}
              />
            </div>
            ...
            <div className="orderAdminStatus">{statusForm(order.state)}</div>
          </div>
          <div className="folding-pannel filter">
            <div className="containerAdminFilterOrderDetail">
              <div className="orderAdminFilterDetailId" onClick={sortId}>
                <DivText content="Subregistro" />
              </div>
              <div className="orderAdminFilterDetailName" onClick={sortName}>
                <DivText content="Producto" />
              </div>
              <div
                className="orderAdminFilterDetailQuantity"
                onClick={sortQuantity}
              >
                <DivText content="Cantidad" />
              </div>
              <div className="orderAdminFilterDetailPrice" onClick={sortPrice}>
                <DivText content="Precio Unidad" />
              </div>
              <div className="orderAdminFilterDetailCost" onClick={sortCost}>
                <DivText content="Costo por Item" />
              </div>
            </div>
          </div>
          {ordersDetails?.map((orderDetail) => {
            return (
              <div
                ...
                key={`OrderDetail-${orderDetail.id}-${order.id}`}
              >
