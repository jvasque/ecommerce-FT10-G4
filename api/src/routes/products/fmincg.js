...
async function fmincgT(f, arguments, X, options){
    let length = 100
    if(options && options.maxIter){
        length = options.maxIter
    }
    let {Ynorm, R, F, nu, nm, nf, lambda} = arguments
    const RHO = 0.01
    const SIG = 0.5
    const INT = 0.1
    const EXT = 3.0
    ...
    let B = 0
    let z2 = 0
    let arr = await f(X, Ynorm, R, F, nu, nm, nf, lambda)
    let f1 = arr[0]
    let df1 = arr[1]
    i = i + (length<0)
    let s = tf.mul(df1, -1)
    ...
    while(i < Math.abs(length)){
        ...
        X = tf.add(X, tf.mul(z1, s))
        arr = await f(X, Ynorm, R, F, nu, nm, nf, lambda)
        let f2 = arr[0]
        let df2 = arr[1]
        i = i + (length<0)
        let d2 = tf.matMul(df2, s, true, false).reshape([])
